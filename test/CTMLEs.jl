...
facts("Test CTMLEs") do
    ...
    context("making chunk subsets") do
        qc = CTMLEs.make_chunk_subset(logitQnA1, logitQnA0, w, a, y, ATE(), 1:3, [0.0, 1.0], false, Logistic())
        @fact qc --> is_a(Qchunk)
        @fact CTMLEs.nobs(qc) --> 3
        @fact qc.risk --> CTMLEs.risk(Qmodel(Logistic(), logitQnA1[1:3], logitQnA0[1:3]), a[1:3], y[1:3], ATE(), false)
        qc_pen = CTMLEs.make_chunk_subset(logitQnA1, logitQnA0, w, a, y, ATE(), 1:3, [0.0, 1.0], true, Logistic())
        @fact qc_pen.risk --> Inf
    end

    context("FluctuationInfo") do
        g1 = LR(Logistic(), zeros(1), [1], false)
        g2 = LR(Logistic(), zeros(2), [1, 3], false)
        g1b = LR(Logistic(), zeros(2), [1, 5], false)
    ...
    context("It runs...") do
        ...
        est4 = ctmle(logitQnA1, logitQnA0, w, a, y, searchstrategy=CTMLEs.PreOrdered(CTMLEs.ManualOrdering([2,3,5,4,6])))
