...
facts("Test CTMLEs") do
    ...
    context("making chunk subsets") do
        qc = CTMLEs.make_chunk_subset(logitQnA1, logitQnA0, w, a, y, ATE(), 1:3, [0.0, 1.0], false, IntSet(), IntSet(1:p))
        @fact qc --> is_a(Qchunk)
        @fact CTMLEs.nobs(qc) --> 3
        @fact qc.risk --> CTMLEs.risk(Qmodel(logitQnA1[1:3], logitQnA0[1:3]), a[1:3], y[1:3], ATE(), false)
        qc_pen = CTMLEs.make_chunk_subset(logitQnA1, logitQnA0, w, a, y, ATE(), 1:3, [0.0, 1.0], true, IntSet(), IntSet(1:p))
